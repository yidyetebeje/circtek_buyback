...
import { tenants, users, warehouses } from "./circtek.schema"
...
export const shops = mysqlTable("shops", {
	...
	owner_id: bigint("owner_id", { mode: 'number', unsigned: true }).references(() => tenants.id).notNull(),
	logo: text("logo"),
...
export const shop_locations = mysqlTable("shop_locations", {
	...
	(table) => [
		...
		index("shop_locations_warehouse_id_idx").on(table.warehouse_id),
		unique("shop_locations_warehouse_id_unique").on(table.warehouse_id),
	]);
