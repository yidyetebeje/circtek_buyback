...
export const warehouse_routes = new Elysia({ prefix: '/warehouses' })
	...
	.patch('/:id', (ctx) => {
		...
		return controller.update(Number(params.id), body as any, currentRole === 'super_admin' ? undefined : (currentTenantId as number))
	}, { body: WarehouseUpdate, detail: { tags: ['Warehouses'], summary: 'Update warehouse' } })
	.delete('/:id', (ctx) => {
		const { params, currentRole, currentTenantId } = ctx as any
		return controller.remove(Number(params.id), currentRole === 'super_admin' ? undefined : (currentTenantId as number))
