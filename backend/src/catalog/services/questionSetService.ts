...
export class QuestionSetService {
  ...
  async getAllQuestionSets(
    ...
    return questionSetRepository.findAll(page, limit, orderBy, order, clientId, search);
  }
  ...
  async getQuestionSetByInternalName(internalName: string, clientId: number) {
    const questionSet = await questionSetRepository.findByInternalName(internalName, clientId);
    if (!questionSet) {
  ...
  async createQuestionSet(data: TQuestionSetCreate, clientId: number) {
    // Check for duplicate internal name within the same client
    const existingSet = await questionSetRepository.findByInternalName(data.internalName, clientId);
    if (existingSet) {
      throw new BadRequestError(`Question set with internal name '${data.internalName}' already exists for this client`);
    }
  ...
  async updateQuestionSet(id: number, data: TQuestionSetUpdate) {
    ...
    if (data.internalName && data.internalName !== questionSet.internalName && data.client_id) {
      const existingSet = await questionSetRepository.findByInternalName(data.internalName, data.client_id);
  ...
  async createQuestionSetWithQuestions(data: TQuestionSetWithQuestions, clientId: number) {
    // Create the question set first
    const questionSet = await this.createQuestionSet(data, clientId);
    if (!questionSet) {
  ...
  async createQuestionSetWithQuestionsAndTranslations(data: TQuestionSetWithQuestionsAndTranslations, clientId: number) {
    // Create the question set first
    const questionSet = await this.createQuestionSet(data, clientId);
    if (!questionSet) {
      throw new NotFoundError("Failed to create question set");
