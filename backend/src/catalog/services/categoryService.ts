...
export class CategoryService {
  ...
  async getAllCategories(
    ...
    return categoryRepository.findAll(page, limit, orderBy, order, clientId);
  }
  ...
  async getCategoryBySlug(slug: string, clientId: number) {
    const category = await categoryRepository.findBySlug(slug, clientId);
    if (!category) {
  ...
  async createCategory(data: TCategoryCreate) {
    // Check for duplicate sef_url within the same client
    if(data.sef_url){
      const existingCategory = await categoryRepository.findBySlug(data.sef_url, data.client_id);
      if (existingCategory) {
        throw new Error(`Category with URL '${data.sef_url}' already exists for this client`);
      }
    }
  ...
  async updateCategory(id: number, data: TCategoryUpdate) {
    ...
    if (data.sef_url && data.sef_url !== category.sef_url && data.client_id) {
      const existingCategory = await categoryRepository.findBySlug(data.sef_url, data.client_id);
