...
import { NotFoundError, InternalServerError, BadRequestError } from "../utils/errors"; // Assuming error utility exists
import { db } from "../../db";
import { brandTranslations, languages } from "../../db/schema/catalog";
...
export class BrandService {
  async getAllBrands(
    ...
    return brandRepository.findAll(page, limit, orderBy, order, clientId);
  }
  ...
  async getBrandBySlug(slug: string, clientId: number) {
    const brand = await brandRepository.findBySlug(slug, clientId);
    if (!brand) {
      throw new NotFoundError(`Brand with slug ${slug} not found for client ${clientId}.`);
    }
    return brand;
  }
  ...
  async updateBrand(id: number, data: TBrandUpdate) {
    ...
    if (translations) {
        ...
        if(translationsToDelete.length > 0){
           ...
           const transRecordsToDelete = await db.query.brandTranslations.findMany({
               where: and(
                   eq(brandTranslations.brand_id, id),
                   inArray(brandTranslations.language_id, translationsToDelete)
               ),
               columns: { id: true }
           });
           if (transRecordsToDelete.length > 0) {
                await db.delete(brandTranslations).where(inArray(brandTranslations.id, transRecordsToDelete.map(t => t.id)));
           }
        }
