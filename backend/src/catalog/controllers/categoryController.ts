...
export class CategoryController {
  async getAllCategories(ctx: Context<{ query: Static<typeof PaginationQuerySchema> }> & { user: JwtUser }) {
    ...
    const categories = await categoryService.getAllCategories(page!, limit!, orderBy!, order!, effectiveClientId);
    return {
  ...
  async getCategoryBySlug(ctx: Context<{ params: Static<typeof CategorySlugParamSchema> }>) {
    const { params } = ctx;
    const category = await categoryService.getCategoryBySlug(params.slug, params.clientId);
    return {
  ...
  async createCategory(ctx: Context<{ body: TCategoryCreate }> & { user: JwtUser; error: Context['error'] }) {
    const { body, user, error: ElysiaError } = ctx;
    const data = { ...body };
    const clientId = getClientId(user);
    if (clientId === undefined) {
      return ElysiaError(400, 'Client ID could not be determined from user information');
    }
  ...
  async createCategoryWithTranslations(ctx: Context<{ body: TCategoryWithTranslationsCreate }> & { user: JwtUser; error: Context['error'] }) {
    const { body, user, error: ElysiaError } = ctx;
    const data = { ...body };
    const clientId = getClientId(user);
    if (clientId === undefined) {
    ...
    data.category = { ...data.category, client_id: clientId };
  ...
  async bulkUpsertTranslations(ctx: Context<{ params: Static<typeof CategoryIdParamSchema>, body: { translations: Array<{ language_id: number; title: string; description?: string; meta_title?: string; meta_description?: string; meta_keywords?: string; }> } }> & { user: JwtUser | null }) {
    try {
  ...
  async getPublishedInShop(
    ...
    options: {
      ...
      order?: 'asc' | 'desc';
      search?: string;
      clientId?: number; // This clientId is for filtering categories by client, if applicable
    },
    ...
    const filterByClientId = options.clientId; 
    ...
    if (filterByClientId !== undefined && isNaN(filterByClientId)) throw new BadRequestError('Invalid client ID.');
