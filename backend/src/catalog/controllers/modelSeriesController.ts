...
import { JwtUser } from "@/middleware/auth"; // Import JwtUser type
...
export class ModelSeriesController {
  async getAll(ctx: Context & { user?: JwtUser }) { // Add user to context type
    try {
      ...
      let effectiveClientId: number | undefined = undefined;
      const queryClientId = ctx.query.clientId ? parseInt(ctx.query.clientId as string, 10) : undefined;

      if(ctx.user && ctx.user.roleSlug !== 'super-admin'){
      ...
      const result = await modelSeriesService.getAllModelSeries(page, limit, orderBy, order, effectiveClientId, search);
      return result;
    } catch (error: any) {
  ...
  async bulkUpsertTranslations(ctx: Context & { user: JwtUser | null }) {
    try {
