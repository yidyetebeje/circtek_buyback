...
export class FAQController {
   ...
   * Create FAQ
   */
  async createFAQ(data: Omit<TFAQCreate, 'tenant_id'>, currentTenantId: number, ctx: Context) {
    try {
      const faq = await faqService.createFAQ({ ...data, tenant_id: currentTenantId });
      ctx.set.status = 201;
      return {
        success: true,
        data: faq,
   ...
   */
  async createFAQWithTranslations(data: Omit<TFAQWithTranslationsCreate, 'tenant_id'>, currentTenantId: number, ctx: Context) {
    try {
      const faq = await faqService.createFAQWithTranslations({ ...data, tenant_id: currentTenantId });
      ctx.set.status = 201;
      return {
        success: true,
        data: faq,
