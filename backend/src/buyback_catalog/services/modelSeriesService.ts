...
export class ModelSeriesService {
  async getAllModelSeries(
    ...
    return modelSeriesRepository.findAll(page, limit, orderBy, order, tenantId, search);
  }
  ...
  async getModelSeriesBySlug(slug: string, tenantId: number) {
    const modelSeries = await modelSeriesRepository.findBySlug(slug, tenantId);
    if (!modelSeries) {
      throw new NotFoundError(`Model Series with slug ${slug} not found for tenant ${tenantId}.`);
    }
    return modelSeries;
  }
  ...
  async createModelSeries(data: TModelSeriesCreate) {
    ...
    // Duplicate check based on SEF URL (slug) + tenant
    const slug = modelSeriesData.sef_url ?? toSafeUrl(modelSeriesData.title);
    const existing = await modelSeriesRepository.findBySlug(slug, modelSeriesData.tenant_id);
    if (existing) {
      throw new ConflictError(`A model series with the slug "${slug}" already exists for this tenant.`);
    }
  ...
  async updateModelSeries(id: number, data: TModelSeriesUpdate) {
    ...
    if (title !== undefined || sef_url !== undefined) {
      ...
      if (newSlug) {
        const duplicate = await modelSeriesRepository.findBySlug(newSlug, existingModelSeries.tenant_id);
    ...
    // Handle translations
    if (translations) {
        const existingTranslationIds = (await modelSeriesRepository.findTranslationsForSeries(id)).map((t: any) => t.language_id);
        const updatedTranslationLangIds = translations.map((t: any) => t.language_id);

        // 1. Delete translations that are not in the new list
        const translationsToDelete = existingTranslationIds.filter((langId: any) => !updatedTranslationLangIds.includes(langId));
        if(translationsToDelete.length > 0){
           const transRecordsToDelete = await db.select({ id: model_series_translations.id })
               .from(model_series_translations)
               .where(and(
                   eq(model_series_translations.series_id, id),
                   inArray(model_series_translations.language_id, translationsToDelete)
               ));
           if (transRecordsToDelete.length > 0) {
                await db.delete(model_series_translations).where(inArray(model_series_translations.id, transRecordsToDelete.map((t: any) => t.id)));
           }
        }
        ...
        for (const transData of translations) {
            ...
            if (existingTranslation && existingTranslation.length > 0) {
                // Update existing
                await modelSeriesRepository.updateTranslation(existingTranslation[0].model_series_translations.id, transData);
            } else {
                // Create new
      ...
      await Promise.all(
        chunk.map(async (translation) => {
          try {
            ...
            const languageExists = await db.select().from(languages).where(eq(languages.id, translation.language_id)).limit(1);
            if (!languageExists[0]) {
              results.errors.push({
