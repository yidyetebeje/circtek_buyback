...
export class WorkflowFormComponent {
  ...
  title = computed(() => (this.isEditMode() ? 'Edit Workflow' : 'Add Workflow'));
  subtitle = computed(() => (this.isEditMode() ? 'Update workflow' : 'Create a new workflow'));
  submitLabel = computed(() => (this.isEditMode() ? 'Update Workflow' : 'Create Workflow'));
  ...
  actions = computed<FormAction[]>(() => [
    ...
    { label: this.isEditMode() ? 'Update Workflow' : 'Create Workflow', type: 'submit', variant: 'primary' },
  ]);
  ...
  private loadRecord(id: number) {
    ...
    this.api.getWorkflow(id).subscribe({
      ...
      error: () => { this.loading.set(false); this.router.navigate(['/management'], { queryParams: { tab: 'workflows' } }); },
    });
  }
  ...
  onFormSubmit(raw: any) {
    ...
    if (this.isEditMode()) {
      this.api.updateWorkflow(this.recordId()!, payload).subscribe({
        next: () => { this.submitting.set(false); this.router.navigate(['/management'], { queryParams: { tab: 'workflows' } }); },
        error: (err) => { this.errorMessage.set(err?.error?.message || err?.message || 'Failed to update'); this.submitting.set(false); },
      });
    } else {
      this.api.createWorkflow(payload).subscribe({
        next: () => { this.submitting.set(false); this.router.navigate(['/management'], { queryParams: { tab: 'workflows' } }); },
  ...
  onActionClick(event: { action: string; data?: any }) {
    if (event.action === 'Cancel') this.router.navigate(['/management'], { queryParams: { tab: 'workflows' } });
  }
}
