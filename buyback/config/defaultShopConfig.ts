...
// Helper to load shop config based on shop ID
export async function getShopConfig(shopId?: string): Promise<ShopConfig> {
  ...
  // Here we would normally fetch the specific shop config
  // For demonstration, we'll return a slightly modified default
  return {
    ...defaultShopConfig,
    shopId,
    shopName: `${shopId.charAt(0).toUpperCase() + shopId.slice(1)} Buyback`,
    theme: {
      ...defaultShopConfig.theme,
      primary: shopId === "apple" ? "#000000" : 
               shopId === "samsung" ? "#1428a0" : 
               defaultShopConfig.theme.primary,
    }
  };
} 